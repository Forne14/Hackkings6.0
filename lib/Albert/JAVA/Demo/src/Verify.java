package lib.Albert.JAVA.Demo.src;
...
public class Verify {
     ...
     */
    /**
     * The constructor of class Verify
    ...
    public Verify(String text)throws Exception{
        this.text = text;
        //Generate QRcode
       // getQRcode(1);
        //Verify QRcode
...
//
     ...
     * @return void
     */
    public  void getQRcode(int num)throws Exception{
        if(num>0) {
            for (int i = 0; i < num; i++) {
                Crypt crypt = new Crypt(text);
                crypt.main();
            }
        }
    }
    ...
    */
    ...
    If_True(String Keyfilename,String Textfilename,String QRcodetext)throws Exception{
        //初始化两个bufferedReader
        ...
        try {
          ...
          //  String QRcodetext = QRcode.decode(filename);
            ...
            System.out.println("-------------------------------------------------------------------------");
         //   System.out.println("是否使用DES加密并 且使用RSA加密DES密匙？若是请输入Yes,若不是请输入任意字符跳过...");
